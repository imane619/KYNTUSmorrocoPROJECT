...
{
    c.SwaggerDoc("v1", new OpenApiInfo { Title = "ShiftMaster Absence API", Version = "v1" });
...
{
    var db = scope.ServiceProvider.GetRequiredService<AbsenceDbContext>();
...
app.MapControllers();
app.MapGet("/health", () => Results.Ok(new { status = "healthy", service = "Absence" }));
